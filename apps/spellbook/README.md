...
拼装规则在 `shared/prompt.mjs`：描述（去掉 `==机制==` 标记）+ 靠什么成立 + 示例代码 +
容易失效的地方 + 出处。代码里站内自己的 `/* @mechanism 说明 */` 批注会被转成普通注释，
...
于是每一页都只有一条正文左边缘（370）和一条右边缘（1250）：检索区、花饰、
...
| 魔杖 | 查词口、花饰 | **器械**。一句话就是咒语，魔杖就是那句话 |
| 花饰 | 序言与正文之间 | 印刷书的分隔纹（细线夹一支魔杖） |
| 方钻 | 章首饰线的起点 | 章首纹 |

> **刊头为什么放书、查词口为什么放杖。** 刊头那枚答的是「这是什么」（这本书自己），
> 查词口那枚答的是「你怎么用它说话」（器械）。原先刊头也是魔杖，两处撞了同一个记号，
> 一页上出现两次的符号就不再是记号了。换成书之后，同一个页面里书是物、杖是器械，
> 各表一事。
>
> 书是**合着**画的，不是摊开的：22px 下摊开的书要画两页纸的弧度，那点弧度会糊成
> 两条并列的横线，看着像等号。合着的书只有三条直线（封面框、书脊、书扣），小尺寸
> 下轮廓反而最清楚。书扣那一横是故意捅出封面右边线的 —— 不捅出去，这本书读起来
> 就是一个方框。
>
> 线稿只有两种笔画：`mark-line`（描边）与 `mark-spark`（填实的火星）。三个装置共用
> 这两种，所以名字不挂在某一个装置上 —— 原先叫 `wand-shaft` / `wand-spark`，
> 书身上于是长着「魔杖的零件」。
