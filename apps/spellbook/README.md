...
| 演示的一层 | 示例代码 | 带**它自己那份语言的名字**，随时可被改写 |

具体到文本上：

- 开场三句把「什么可搬、什么不可搬」先说清：「要拿走的是效果、机制、边界这三样；
  代码只是其中一种落法」「用哪个栈…取决于你手里这个项目：先看清它，再按它改写」
  「示例里的类名、尺寸、结构都是随手取的，不是接口」。
- 示例的语言名**从数据里现算**（`entry_code.lang`），不写死。哪一天库里收了
  React、Vue、shader，那一句自己就跟着变，不需要有人记得回来改生成器。
  查不到的语言**用原样**——这个兜底是故意的，不该因为漏配一个键就把人家的语言名吞掉。
