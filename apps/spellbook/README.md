...
| `--rail` 前的刊头首格 | `calc(--rail - --masthead-gap)` | 让刊头里的查词口也落在 370 上 |
...
刊头排成三格，格子仍从边栏推出来（首格 `calc(--rail - --masthead-gap)`），
所以查词口（或内页的当前条目名）的左边缘正好落在正文那条线 370 上，
与下面的筹码、结果、条目齐平 —— 吸顶的时候也不会跳。

三样**各就各位，不靠自动排布**。自动排布在内页上会出错：内页刊头只有书名与
开关，第三格空着就塌成 0 宽，开关于是落到第二格（1fr）的末尾，右边缘停在
1228 而不是 1250，比首页短 22px；来回点首页与内页，开关就会左右跳一下。
...
窄屏分两档：≤860px 藏掉副标题、压紧行距，三样仍并一行（刊头 79px）；
≤520px 退回两行——书名与开关一行、查词口独占一行。挤在一行时查词口只剩
88px，敲不进几个字，等于没有；宁可吸顶那坨高一点（112px），也不能给一个
用不了的检索口。
