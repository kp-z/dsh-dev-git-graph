...
二十页的列表里，页码按钮永远是七个：首页、末页、当前页左右各两页；被跳过的地方放省略号，所以整排固定九个格子。
...
滑动时机是第二个容易做错的地方：这里让窗口每翻一页就平移一格，好处是当前页始终停在正中间，用户的手指不用挪、动的是两边的数字。另一种取向是「只在越过窗口中心时才滑」，那样当前页会在行内来回跑——两种都成立，但必须二选一。
...
- 滑动时机要和选定的一致：这条是「每页平移一格、当前页永远居中」，另一种是「只在越过窗口中心时才滑」。混用会出现当前页一会儿居中、一会儿贴边，手指会跟丢。
