 ...
 * window.__DSH_GG_BOOT__ 由宿主在 index.html 注入：{ apiBase, repo, initialState, globalState, workspaceState }。
 */
interface DshGgBootData {
	apiBase: string;
	repo: string;
	initialState: GG.GitGraphViewInitialState;
	globalState: GG.GitGraphViewGlobalState;
	workspaceState: GG.GitGraphViewWorkspaceState;
}
const DSH_GG_BOOT = (window as unknown as { __DSH_GG_BOOT__?: DshGgBootData }).__DSH_GG_BOOT__;

const initialState: GG.GitGraphViewInitialState = DSH_GG_BOOT !== undefined ? DSH_GG_BOOT.initialState : ({} as GG.GitGraphViewInitialState);
const globalState: GG.DeepReadonly<GG.GitGraphViewGlobalState> = DSH_GG_BOOT !== undefined ? DSH_GG_BOOT.globalState : ({ alwaysAcceptCheckoutCommit: false, issueLinkingConfig: null, pushTagSkipRemoteCheck: false } as GG.GitGraphViewGlobalState);
const workspaceState: GG.DeepReadonly<GG.GitGraphViewWorkspaceState> = DSH_GG_BOOT !== undefined ? DSH_GG_BOOT.workspaceState : ({ findIsCaseSensitive: false, findIsRegex: false, findOpenCommitDetailsView: false } as GG.GitGraphViewWorkspaceState);
...
/** VS Code 兼容桥：postMessage -> POST /dsh-dev-git-graph/gg/api，响应通过 window message 回投（保持 main.ts 的监听协议）。 */
	...
	return {
		getState: () => {
			try {
				...
				return raw !== null ? (JSON.parse(raw) as WebViewState) : null;
			} catch {
				return null;
			}
		},
		postMessage: (message: GG.RequestMessage) => {
			void (async () => {
				try {
					...
					const data = (await res.json()) as GG.ResponseMessage;
					window.postMessage(data, window.location.origin);
				} catch (e) {
					window.postMessage({ command: (message as { command: string }).command, error: String(e instanceof Error ? e.message : e) }, window.location.origin);
				}
			})();
		},
...
/** 主题同步：接收宿主（client.js）通过 postMessage 推送的 { dark, tokens }。
 * dark -> 切换 <html data-ds-dark-theme>（themeCss 的两套 :root 变量随之切换）；
 * tokens -> 以内联变量写入宿主解析后的真实 --dsw-alias-* 值（覆盖 CSS 静态兜底）。 */
function dshGgApplyTheme(dark: boolean, tokens: { [name: string]: string }): void {
	...
	for (const name in tokens) {
		if (Object.prototype.hasOwnProperty.call(tokens, name)) {
			...
			} catch { /* ignore invalid token names */ }
		}
	}
}

window.addEventListener('message', (e: MessageEvent) => {
	const data = e.data as { type?: string, dark?: boolean, tokens?: { [name: string]: string } };
	if (data !== null && typeof data === 'object' && data.type === 'dsh-dev-gg-theme') {
		dshGgApplyTheme(!!data.dark, data.tokens ?? {});
	}
});

// 主动请求一次当前主题（宿主 iframe load 监听可能晚于本脚本执行）。
